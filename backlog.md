...
| 日期 | 事项 |
|---|---|
| 2026-09-02 | **T-2026-0902-006 gitguard 合 main 保护守卫交付(发布 v1.1.0,进观察窗)**:dev 不提交连环翻车(003-S3/S5、006-S1 守卫拦自家)根因=合 main 无守卫;交付 gitguard 守卫(白名单签名+脏树拦截,含 .orc-base 记号豁免)+ `orc guard pre-merge` CLI + playbook 接线;QA 20 用例全绿,发布员全量两轮 377 绿/5 沙箱基线存量/2 deselected 零新增,check-pool-load 0 bad;发布自身 dogfood:并发落 main 的非白名单提交 e0a4412 被守卫拦下,reword 收口 fix(T-2026-0902-006)(4cb23c8)后 PASS;deploy local exit 0 冒烟 200(dashboard 6676→20184),发布记录五章齐备、回滚 v1.0.0;tag v1.1.0(覆盖 v1.0.0 后 22 提交,021 去重/019 k3-c/runner 硬化一并打标)。注:origin push 待环境补推(沙箱凭据助手不可用,openssl 只读可达);观察窗内 sre 验证首个真实 loop 合 main 全程过守卫(loop_merge 事件+签名入白名单) |
...
| 2026-09-01 | **T-2026-0901-021 create_incident 去重交付,进观察窗**:007 重试循环雪崩 9+ 张重复单根因=自动建单无去重;新增 incident.py 收口(deploy/runner 双路径),未关闭 incident 复用不新建;pytest 362 绿;现场清理:011-015/017/018/020 关闭归并 010。合并 5b17c2a |
