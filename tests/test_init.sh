...
bash plugin/scripts/init-project.sh --dir "$FIX" --name "demo-proj" --type "单元测试夹具" \
...
if bash plugin/scripts/init-project.sh --dir "$FIX" --name "demo-proj" --type "x" 2>/dev/null; then
  echo "FAIL: 二次运行未拒绝覆盖"; fail=1
fi
# --force 显式重建必须成功
bash plugin/scripts/init-project.sh --dir "$FIX" --name "demo-proj" --type "x" --force >/dev/null \
...
if bash plugin/scripts/init-project.sh --dir "$CON" --name "x" --type "x" 2>/dev/null; then
  echo "FAIL: 容器目录未拒绝"; fail=1
fi
# 容器内的新子目录必须可初始化,且自动补写 settings.json
bash plugin/scripts/init-project.sh --dir "$CON/proj-b" --name "proj-b" --type "x" >/dev/null \
...
bash plugin/scripts/init-project.sh --dir "$CON/proj-c" --name "proj-c" --type "x" >/dev/null \
