...
# 1. JSON 合法性
python -c "import json;d=json.load(open(r'$ROOTW/plugin/.claude-plugin/plugin.json',encoding='utf-8'));assert d.get('name')=='ai-factory'" \
  || err "plugin.json 非法"
python -c "import json;json.load(open(r'$ROOTW/plugin/hooks/hooks.json',encoding='utf-8'))" \
  || err "hooks.json 非法"
...
d=json.load(open(r'$ROOTW/plugin/hooks/hooks.json',encoding='utf-8'))
...
" | tr -d '\r'); do
  [ -f "$ROOT/plugin/$p" ] || err "hooks.json 引用缺失脚本: $p"
done
...
for t in stack-profile.yaml claude-md.template.md backlog.template.md state.template.json \
  ...
  [ -f "$ROOT/plugin/templates/$t" ] || err "缺模板: $t"
done
python -c "import json;json.load(open(r'$ROOTW/plugin/templates/state.template.json',encoding='utf-8'))" \
  || err "state.template.json 非法"

# 7. 门禁清单
for n in 1 2 3 4 5; do
  [ -f "$ROOT/plugin/templates/gate-checklists/phase$n-gate.md" ] || err "缺门禁清单 phase$n"
done
